...
public class NotificationChannelRoutingService {
    ...
    public NotificationChannelRoutingService(NotificationSettingRepository settings, UserNotificationPreferenceRepository preferences, DesktopDeviceRepository devices, PushSubscriptionRepository subscriptions, PushSender pushSender, RealtimeConnectionService realtime) {
        this.settings = settings; this.preferences = preferences; this.devices = devices; this.subscriptions = subscriptions; this.pushSender = pushSender; this.realtime = realtime;
    }
    ...
    public void deliver(NotificationCreatedEvent event) {
        for (Long userId : event.recipientUserIds()) {
            ...
            boolean webPush = !subscriptions.findByUserIdAndIsActiveTrue(userId).isEmpty();
    ...
    private void sendPush(NotificationCreatedEvent event, Long userId) {
        try { pushSender.sendPush(event.notificationId(), userId, event.title(), event.content(), event.actionUrl()); }
